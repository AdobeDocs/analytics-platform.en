...
Match by container handles the common cases without requiring you to manually configure the key fields.
...
Imagine you have the following event, profile, account and opportunity lookup datasets.
...
And you want to report on the opportunity value for each account you can have the following results

| Account name | Opp. value (OO) | Opp. value (SAO) | Opp. value (POPO) |
|---|---:|---:|---:|
| Acme | $3M | $11M | $4M |
| BigCo | $7M | $23M | $7M |

Thes results are based on the selected join path per account ID:

| Join path ID | Join path | Explanation |
|---|---|---|
| OO | Events (Opportunity ID) → Opportunity Lookup (Opportunity ID) | Acme: O-432 ($2M) + O-543 ($1M) = $3M<br>BigCo: O-876 ($7M) |
| SAO | Events (Opportunity ID/Account ID) → Opportunity Lookup (Account ID/Opportunity ID) | Acme: O-432/A-123 ($2M) + O-543/A-123 ($1M) + O765/A-123 ($8M) = $11M<br/>BigCo: O-876/A-456 ($7M) + O-987/A-456 ($16M) = $23M |
| POPO | Events (Opportunity ID/Person ID) → Opportunity / Person (Person ID/Opportunity ID) →  Opportunity Lookup (Account ID/Opportunity ID) | Acme: O-432/P-ABC ($2M) + O-543/P-ABC ($1M) + O-543/P-EFG ($1M) = $4M<br/>BigCo: O-876/P-LMN ($7M) | 
