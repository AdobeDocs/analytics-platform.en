...
| [!UICONTROL Records available] widget | Represents the total number of rows available for reporting, **for the entire connection**. This count is independent of any calendar settings or dataset selection. This count does not change when you change the dataset in the dataset selector. ( Note that there is a latency of 1-2 hours to get the data to appear in reporting, once it is ingested.)   |
