...
>abstract="The average amount of storage consumed by each row of data ingested and stored in Customer Journey Analytics (in KB) for the current month, with a percentage change compared to the previous month."
...
* The **[!UICONTROL Key usage metrics]** panel that displays:
  ...
  * Four summary visualizations that display total and percentual changes from the previous month for:
    ...
    * **[!UICONTROL Average row size]**. The average amount of storage consumed by each row of data ingested and stored in Customer Journey Analytics (in KB) for the current month, with a percentage change compared to the previous month.
