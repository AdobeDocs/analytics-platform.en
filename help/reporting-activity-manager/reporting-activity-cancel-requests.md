...
You can cancel all requests that are associated with one or more applications. Applications include the following: 
...
1. (Optional) To restrict future requests for a given time period, enable the option to [!UICONTROL **Restrict subsequent requests**], then choose from the following options:
   ...
   |Option | Function | 
   |---------|----------|
   | [!UICONTROL **User & project**] | Selected applications will be temporarily restricted from any reporting requests made by the associated users and projects. |
...
### When subsequent requests are restricted
