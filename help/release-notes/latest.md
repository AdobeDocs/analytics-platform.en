...
| **Updated CJA learning page with CJA-centric content** | The [!UICONTROL Learning] tab on the Customer Journey Analytics landing page now contains content specific to CJA, including content focused on transitioning to CJA from Adobe Analytics. The following additional enhancements are also available on the Learning tab:<ul><li>Improved design that shows more learning content on a single page with improved navigation</li><li>Ability to personalize learning content by experience level (Beginner, Intermediate, and Advanced)</li></ul>Previously, the [!UICONTROL Learning] tab in CJA contained identical information to the [!UICONTROL Learning] tab in Adobe Analytics. |  | June 30, 2023 |
