...
| **Permissions enhancement for project-only [!UICONTROL Workspace] components** | Previously, if one user (User A) shared a project with another user (User B), and gave User B edit access to the project, User B would be able to edit the project. However, User B would not be able to edit [!UICONTROL quick filters] embedded in the project. That limitation is now removed - User B can edit [quick filters](/help/components/filters/quick-filters.md) and other project-only components that are embedded in the shared project. |  | April 17, 2024 |
