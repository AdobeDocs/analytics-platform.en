...
| **Report Builder usage is now included in the "Used in" column in the Calculated metrics manager and the Filters manager** | When viewing the **Used in** column in the Calculated metrics manager or the Filters manager, usage data is now available for the Report Builder.<p>Previously, usage data in the Filters manager was available only for Alerts, Projects, Scheduled Projects and Calculated metrics; while usage data in the Calculated metrics manager was available only for Alerts, Projects, and Scheduled Projects.</p> |  | Late march or early April |
