...
Controls how far back Customer Journey Analytics looks when processing the data feed delivery. This setting does not alter the frequency window (hour or day). 
...
Lookback window used to determine persistence and attribution. IF in the data view they have selected a container. We don' thave a reporting date range in DF. 

If they have a dimension that is Last-touch person, normally we would use the Workspace reporting date range, but we don't have one in DF. If they set something explicit and it's longer than our lookback, we'll trim it to what our lookback is. This is what Bret is worried about. Because if it's 90 days, then we'll trim it to whatever the lookback is. 

Reporting date range in Workspace is "This month", so maybe we should use that for DF. 

If you have a campaign with 90-day expiration, and then someone puts a 1-day lookback on the df, then the campaign has a 1-day lookback. Whereas in WS if the date range was 1 day, the lookback would still go back farther. We did this because in DF we're not aggregating, so we have a lot more memory pressure. (performance). 90 days of data for every column would increase the cost a lot. If it's longer than the cutoff used in the feed, then we would pull that longer date for every dimension in the feed. And then chopping it off to give us just that hour. 

Customers can get around this by extending the lookback to 90 days. 

If the lookback could be column-specific and not the whole report. (would need to request that Puck do this)

Concern is having data feed data differ from Workspace. 

Think of the Lookback window for DF as the normal reporting date range for Workspace. 

If we didn't have the memory performance constraint, and we had the non-chopping, then it would essentially work like WS. 

What are all of the things that are going to make the data different between WS and DF?
* General differences between AA and CJA (is there a doc on this already?)
* Stitching (every time you run a stitching replay, it will change data that we already shipped to them, so the numbers between WS and DF will diverge over time) (Historical data change that's displayed in Workspace)
* Late-arriving hits - Because you won't get all of the late-arriving hits in DF, but you will get them in WS because it's report-time. 
* Batching - If you send in a big batch that's a month's worth of data, you're only going to get a little bit of it (an hour or day), because it's timestamped over the course of 30 days. 
