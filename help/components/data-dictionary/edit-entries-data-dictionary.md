...
1. (Optional) Select the **Filter** icon ![Data Dictionary Filter icon](https://spectrum.adobe.com/static/icons/workflow_18/Smock_Filter_18_N.svg), then select any of the following filter options to filter the list of components:
   ...
   | **[!UICONTROL Created by Adobe]** <!-- I don't see this option-->| Show only components that were created by Adobe. For example, **[!UICONTROL Adobe Target]**. Components that were created by an administrator or another user in your organization are not shown. |
