...
Some SQL functionality is only partially supported and will not return the same results you would see with other databases.  This SQL functionality is used in the SQL generated by various BI tools, but we don't have an exact match for the functionality in Customer Journey Analytics.  Because of this, we have focused on a limited implementation that only covers the minimum BI tool usage without throwing errors.
