...
>[!IMPORTANT]
>
>You should **not** use the **[!UICONTROL Create dataset from CSV]** workflow for ad hoc event or summary data. The ad hoc schema that is generated by the workflow defines all fields to be of type String which you cannot modify afterwards. When you add an event and summary dataset to a connection, this type of dataset requires the definition of at least one field as a timestamp field with type DateTime. <br/>To use ad hoc event or summary data, you should first [use the API to create an ad hoc schema](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/tutorials/ad-hoc#token_type=bearer&expires_in=43197438) and then use the [Create dataset from schema](https://experienceleague.adobe.com/en/docs/experience-platform/catalog/datasets/user-guide#schema) workflow.
