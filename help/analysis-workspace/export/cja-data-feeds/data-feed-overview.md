...
| Lookback window (Adobe Analytics only) |(All visits start at the beginning of that window. You can't have visits that start before it.) The cutoff for late or out-of-order hits that are delivered outside of the **[!UICONTROL Delivery frequency]** that is selected (such as **[!UICONTROL Hour]** or **[!UICONTROL Day]**).<p>Not used for persistence, sessions, or dimensions (these are included in the raw data that is collected).</p> | Included in the **[!UICONTROL Reporting window]**. | Supported<p>Example: 23 hours</p> |
