...
1. In the [!UICONTROL **New full table export**] dialog box, specify the following information:
   ...
   | Data structure | Displays the Freeform table that you are exporting. You can modify the data structure by dragging components from the left panel to the table. You can apply a filter by dragging a component into the filter area. The table dynamically updates as you add components to the canvas.  <p>Any segments that were applied to the full table in the project appear at the top of each individual column in the table. You can include up to 10 columns.</p> |
