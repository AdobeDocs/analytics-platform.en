...
The instructions refer to an example data view named public.ares_sql_validation, two example dimensions (Product Name and Product Category) and two example metrics (Purchases and Purchase Revenue). You can change these for your environment where appropriate.


## Connect and list data views

This use case sets up the connection from the BI tool to Customer Journey Analytics and list the available data views to test the connection successfully.
...
1. Access the required credentials and parameters from the Experience Platform Query Service UI.
   ...
   1. Select ![Queries](/help/assets/icons/DataSearch.svg) **[!UICONTROL Queries]** fron the left rail.
...
1. In the **[!UICONTROL PostgreSQL database]** dialog:
   ...
   1. Use ![Copy](/help/assets/icons/Copy.svg) to copy and paste the **[!UICONTROL Database]** value from the Experience Platform **[!UICONTROL Query]** **[!UICONTROL Expiring Credentials]**. Add `?FLATTEN` to the value you paste. For example `prod:cja?FLATTEN`.
...
1. In the **[!UICONTROL PostgreSQL database]** - **[!UICONTROL Database]** dialog:
   ...
   1. Ensure the dropdown menu for **[!UICONTROL Select which level to apply these settings to]** is set to the **[!UICONTROL Server]** that you have defined earlier.
   1. Select **[!UICONTROL Connect]**.
1. In the **[!UICONTROL Navigator]** dialog the data views are retrieved. This can take some time. Once retrieved:
...
1. Open Tableau.
1. Select **[!UICONTROL PostgreSQL]** from the left rail uderneath **[!UICONTROL To a Server]**. If not available, select **[!UICONTROL More...]** and select **[!UICONTROL PostgreSQL]** from the **[!UICONTROL Installed Connectors]**.
...
1. In the **[!UICONTROL PostgreSQL]** dialog, in the **[!UICONTROL General]** tab:
   ...
   1. Use ![Copy](/help/assets/icons/Copy.svg) to copy and paste the **[!UICONTROL Database]** from the Experience Platform **[!UICONTROL Query]** **[!UICONTROL Expiring Credentials]** to the **[!UICONTROL Database]**. Add `%3FFLATTEN` to the value you paste. For example: `prod:cja%3FFLATTEN`.  
   ...
   1. Ensure **[!UICONTROL Require SSL]** is checked.
   1. Select **[!UICONTROL Sign In]**.
   
   You see a **[!UICONTROL Progressing Request]** dialog while Tableau Desktop validates the connection.
1. In the main window, you see in the left panel:
   * The name of connection underneath **[!UICONTROL Connections]**.
   * The name of the database underneath **[!UICONTROL Database]**.
   * a list of tables underneath **[!UICONTROL Table]** in the left rail.
   ![Tableau Connected](assets/tableau-connected.png)
   1. Drag the **[!UICONTROL ares_sql_validation]** entry and drop the entry on the main panel that reads **[!UICONTROL Drag tables]** here.
...
1. In the **[!UICONTROL Data]** pane:
   ...
   You will see a table displaying the occurrences for the current month. For better visibility, enlarge the table visualization using one of the drag handles.
...
1. In the **[!UICONTROL Filters]** pane:
   ...
   You will see the table updated with the applied **[!UICONTROL daterangeday]** filter.

1. In the **[!UICONTROL Visualizations]** pane:

   1. Select the **[!UICONTROL Line chart]** visualization.

   The table is replaced by line chart visualization using the same data as the table.
