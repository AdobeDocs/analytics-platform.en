...
So, instead of attempting to generate the same conversion event from broad segments, focus on building one-for-one customer relationships that last, and analyze the signals and patterns of success. Your conversion rates naturally improve as a by-product of a customer experience-centric strategy.

### Objectives, challenges & differentiators

The customer conversion optimization use case addresses a number of common challenges when trying to achieve your objectives. The combination of Adobe's applications like Real-Time Customer Data Platform, Journey Optimizer and Customer Journey Analytics, and their underlying capabilities, differentiate in overcoming these challenges.

| Objective| Challenge | Differentiation |
|---| ---|---|
| **Build a deep <br/>understanding of<br/>each individual<br/>prospect's journey<br/>& experience.** | Most organizations are challenged with understanding macro-level conversion (revenue-generating events). And then pair that macro-level conversion with the contributing factors for any type of conversion event.<br/><br/>Conversion optimization is also confined to digital channels in isolation (for example, web or mobile). | Adobe provides a more holistic and customer-centric set of applications that enables brands to analyze all the contextual drivers that contribute to positive and compelling experiences. And then  to advance outsized improvement in the conversion events that matter for each customer across channels.<br/><br/>Insights can be actioned throughout the journey to optimize experiences using both trait and behavioral data |
| **Identify & analyze<br/>signals and intent<br/>for each prospect's<br/>conversion needs.** | Difficult to micro- and nano-segment customers to launch targeted campaigns for different cohorts to drive registration or conversion<br/><br/>Brands struggle to use anonymous behaviors to plan impactful experiences across channels. | Adobe allows for advanced segmentation and activation capabilities for pseudonymous profiles that have not yet registered or converted.<br/><br/>As a user advances through their process of consideration to conversion, Adobe applications can work together to create connected omnichannel experiences across advertising, social, email, text, and many more. |
| **Engage and<br/>personalize to each<br/>prospect accordingly<br/>at scale.** | Lack of knowledge of customer interactions on other channels leads to irrelevant or stale messaging and risks long-term economic damage to the brand.<br/><br/>Difficult to maintain a cohesive and connected experience as a user advances through the stages of consideration to conversion | Start customers on their journey with your brand by facilitating one-to-one personalized, omnichannel marketing that acknowledges their interests and engages them in their preferred channels<br/><br/>Use AI modeling to  identify indicators intelligently and route each user down the best path such as a trial download or chat. |
...
Customer conversion optimization, as with any other personalized insights and engagement use case, requires an architecture to support the use case. See the architecture overviews for:
...
* [Experience Platform & applications](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/platform-applications)
* [Experience Platform data flow](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/platform-data-flow)
* and its deployment using the [Experience Platform Web / Mobile SDK & Edge Network](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/deployment/websdk) and the [Application SDKs](https://experienceleague.adobe.com/en/docs/blueprints-learn/architecture/architecture-overview/deployment/appsdk)

## Solutions and Capabilities

To realize and implement your specific customer conversion optimization use cases, consider using the following Adobe applications.

### Customer Journey Analytics
  
Customer Journey Analytics helps you to understand what drives, stops and persuades customers to accomplish key outcomes. Customer Journey Analytics utilizes deep and flexible ad-hoc analytics on populations of fully correlated data to understand and adapt to customer needs in real-time. Customer Journey Analytics can natively connect the insight to Real-Time Customer Data Platform and Journey Optimizer to build an optimization strategy that continuously improves your business.
...
| Customer Lifecycle Optimization | Journey Optimizer gives you what you need to engage, stay engaged, and take steps to actively re-engage when needed appropriately. You want the conversion to be just one of many to come. With Journey Optimizer, you have access to all insights needed to help you understand your customers and what they need. | <ul><li>[Live reports](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/live-report/live-report)</li><li>[Global reports](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/global-report/global-report)</li><li>[Channel reports](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/channel-report/channel-report-gs)</li><li>[Journey reports](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/reports/sharing-overview)</li><li>[Customer Journey Analytics](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/reporting/cja-ajo) </li></ul> |
...
The example below illustrates the customer conversion optimization use case for a fictitious brand Luma and a fictitious customer named Sarah. The example provides a sequence of steps. For each step a description, application usage and detailed instructions are provides.

| Step | Description | Application Usage | Detailed Instructions |
|---|---|---|---| 
| 1 | Sarah is a Luma plus member who has made purchases both online and in store. Recently Sarah has not seen any products of interest and has not made any purchases. | | |
| 2 | You want to identify a customer like Sarah. Customers who have previously purchased yoga apparel, but haven't made a recent purchase. | You use Customer Journey Analytics to define a filter (segment) of customers that have purchased yoga products, but without a recent purchase.<br/>You apply the filter to a Marketing Campaign freeform table to identify which campaigns are driving the most sales for this group. | <ul><li>[Filters overview](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-components/cja-filters/filters-overview)</li><li>[Filter builder](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-components/cja-filters/filter-builder)</li><li>[Filter and sort freeform tables](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-workspace/visualizations/freeform-table/filter-and-sort)</ul> |
| 3 | You also want to market to new visitors and prospects | You use Customer AI to run a propensity score using the Customer Upsell model for customers that are likely to purchase yoga apparel and create a segment. <br/>In Real-Time Customer Data Platform, you review the segment and its options. <br/>You activate the segment to Target. | <ul><li>[Getting started in Customer AI](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-workspace/visualizations/freeform-table/filter-and-sort)</li><li>[Create customer segments with predicted scores](https://experienceleague.adobe.com/en/docs/experience-platform/intelligent-services/customer-ai/user-guide/create-segment)</li><li>[Adobe Target connection](https://experienceleague.adobe.com/en/docs/experience-platform/destinations/catalog/personalization/adobe-target-connection)</li></ul>|
| 4 | Sara is reading the morning news and sees a Luma yoga ad. Sarah accepts cookies, clicks on the ad and noticed the website is personalized for Sarah (using Target). Sarah starts browing the content and the content is progressively personalized with every click. | You use Target to create activities that personalize the visitor's experience. | <ul><li>[Understand Adobe Target Activity Types](https://experienceleague.adobe.com/en/docs/target-learn/tutorials/activities/understanding-the-types-of-activities)</li><li>[Create Experience Targeting Activities](https://experienceleague.adobe.com/en/docs/target-learn/tutorials/activities/create-experience-targeting-activities)</li></ul> |
| 5 | Sarah adds the yoga shorts to her shopping cart... and signs in by using an email address. | You use Real-Time Customer Data Platform to recognize that Sarah has moved from an anonymous to a known profile. | <ul><li>[Browse profiles in Real-Time Customer Data Platform](https://experienceleague.adobe.com/en/docs/experience-platform/rtcdp/profile/profile-browse)</li><li>[Identity graph viewer](https://experienceleague.adobe.com/en/docs/experience-platform/identity/features/identity-graph-viewer)</li></ul> |
| 6 | Not sure about the product, its high price, and not finding the information Sara needs before buying the product, Sarah decides to call the call center for help. The call center rep gives Sarah some additional details, explaining that Sarah is eligible for a Luma plus offer to a free yoga class at the nearby Luma shop.| You use a bespoke call center app, integrated with Real-Time Customer Data Platform (through the Edge Server API), to find all relevant information about Sarah while having the conversation. <br/>You use Decision Management to set up the decisions, placements and offers. | <ul><li>[Edge Network Server API overview](https://experienceleague.adobe.com/en/docs/experience-platform/edge-network-server-api/overview)</li><li>[About Decision Management](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/decisioning/offer-decisioning/get-started-decision/starting-offer-decisioning)</li></ul> |
| 7 | Armed with more information from the call center agent and now 10% off, Sarah decides to complete the purchase. Sarah receives an email confirmation that shows the product purchased info, updated loyalty point status, and the offer for a Free Yoga class at the nearby Luma shop. | You use the profile viewer in Real-Time Customer Data Platform to view Sarah's profile, and any new segment memberships she now qualifies for. <br/>You use Journey Optimizer to build the journey that sends the confirmation email upon purchase.<br/>You use Decision Management to define the rules that send out the specific offer as part of the confirmation email.<br/>You use Customer Journey Analytics flow visualizations to get a better understanding of what product categories are leading to call center calls to see if there are opportunities for call deflection. | <ul><li>[Real-Time Customer Profile UI ](https://experienceleague.adobe.com/en/docs/experience-platform/profile/ui/user-guide)</li><li>[Get started with journeys](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/orchestrate-journeys/journey)</li><li>[Create decision rules](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/decisioning/offer-decisioning/create-components/creating-decision-rules)</li><li>[Get started with rankings](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/decisioning/offer-decisioning/rankings/get-started-rankings)</li><li>[Flow overview](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-workspace/visualizations/flow/flow#cja-components)</li></ul> |
