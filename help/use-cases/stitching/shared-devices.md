...
The example data is a mix of both authenticated and unauthenticated activity for the same device. The challenge is to assign a person to the unauthenticated traffic, so you can perfom a person-centric analysis and Customer Journey Analyics drops the activities that do not have a person id value. To solve this challenge, you have two options: you can use stitching, or you can implement  ECID reset functionality. Both options are discussed in more details in the sections below.
