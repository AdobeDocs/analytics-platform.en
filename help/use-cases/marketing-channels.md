...
>[!NOTE]
>
> In the Analytics Source Connector, if [`va_finder_detail` or `va_closer_detail`](https://experienceleague.adobe.com/docs/analytics/export/analytics-data-feed/data-feed-contents/datafeeds-reference.html?lang=en) is blank in the source report suite, this results in a blank `channel._id` in the XDM data generated by the Analytics Source Connector. Because `channel._id` is a required field in XDM, if `channel._id` is blank then Analytics Source Connector will blank out `channel.typeAtSource` as well. This may result in reporting differences between Adobe Analytics and Customer Journey Analytics.
