...
| Delete a batch from a dataset (in [!UICONTROL Adobe Experience Platform])? | If a batch is deleted from an AEP dataset, the same batch will be removed from any CJA Connections that contain that specific batch. |
