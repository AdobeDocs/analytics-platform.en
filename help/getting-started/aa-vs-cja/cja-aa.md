...
| Processing rules, VISTA rules, Marketing channel processing rules | Supported using Adobe Experience Platform Data Prep functionality for both WebSDK based datasets and data from the Analytics source connector. |
...
| Ability to combine datasets (such as Adobe Analytics report suites) | Customer Journey Analytics allows you to combine data from multiple report suites together as if they were a single report suite in Adobe Analytics. |
| Derived Fields | Derived fields allow report-time transformations to your data. Data can be combined, corrected, or created on the fly and apply retroactively to all reporting. |
