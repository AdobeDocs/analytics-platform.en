...
| Task | Details |
| --- | --- |
| **Step 1: If you are upgrading from Adobe Analytics to Customer Journey Analytics: Choose a upgrade path and send data to Adobe Experience Platform** | There are various paths available when upgrading from Adobe Analytics to Customer Journey Analytics. Each possible upgrade path has its own advantages and disadvantages, and a path that is right for one organization might not make sense for another. <p>To begin upgrading from Adobe Analytics to Customer Journey Analytics, see [Get started with the upgrade to Customer Journey Analytics](/help/getting-started/cja-upgrade/cja-upgrade-getstarted.md). <!-- [Utilizing Adobe Analytics report suite data in Customer Journey Analytics](/help/getting-started/aa-vs-cja/aa-data-in-cja.md) --> </p> |
